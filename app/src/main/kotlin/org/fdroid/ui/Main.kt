...
fun Main(onListeningForIntent: () -> Unit = {}) {
  val navigationState =
    rememberNavigationState(startRoute = NavigationKey.Apps, topLevelRoutes = topLevelRoutes)
  ...
  val entryProvider: (NavKey) -> NavEntry<NavKey> = entryProvider {
    ...
    entry<NavigationKey.Games>(metadata = ListDetailSceneStrategy.listPane("appdetails")) {
      val gamesType = AppListType.Category("游戏", "Games")
      val viewModel =
        hiltViewModel<AppListViewModel, AppListViewModel.Factory>(
          creationCallback = { factory -> factory.create(gamesType) }
        )
      val appListInfo =
        object : AppListInfo {
          override val model = viewModel.appListModel.collectAsStateWithLifecycle().value
          override val list: AppListType = gamesType
